...
* Appropriate feedback should be given to user interaction as required.
...
As can be seen in the following, Hacker Whacker utilised a themed style to present the content. The background is a Matrix like image with a distorted, neon logo supported by a branded image for the team. This header is shown on all pages throughout the site. Two large buttons provide access to the game screen or to the instructions as the user wants. Most of the site looks very similar when the size of the browser window is taken into account and so screenshots are typically shown at only one size here.
...
As documented earlier, the interface is largely similar if viewed on a very small mobile or very large device apart from the amount of space that is available. The following screenshot shows the interface on the minimum mobile screen accounted for, 320x480 in pixels
